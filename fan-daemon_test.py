...
class TestCurvesParse:
    def test_basic(self) -> None:
        curve = Curves.parse("40:15,60:30,80:100")
        assert curve == ((40.0, 15.0), (60.0, 30.0), (80.0, 100.0))

    def test_sorts_by_temp(self) -> None:
        curve = Curves.parse("80:100,40:15,60:30")
        assert curve == ((40.0, 15.0), (60.0, 30.0), (80.0, 100.0))

    def test_empty_returns_none(self) -> None:
        assert Curves.parse("") is None
    ...
    def test_invalid_speed(self) -> None:
        with pytest.raises(ValueError, match="Speed must be 0-100"):
            Curves.parse("40:15,80:150")


class TestCurvesParseSpec:
    def test_gpu_zone(self) -> None:
        key, curve = Curves.parse_spec("gpu-zone=40:15,80:100")
        assert key == ("gpu", -1, -1)
        assert curve == ((40.0, 15.0), (80.0, 100.0))

    def test_gpu_zone0(self) -> None:
        key, _ = Curves.parse_spec("gpu-zone0=40:15,80:100")
        assert key == ("gpu", -1, 0)

    def test_gpu0_zone1(self) -> None:
        key, _ = Curves.parse_spec("gpu0-zone1=40:15,80:100")
        assert key == ("gpu", 0, 1)

    def test_disabled(self) -> None:
        key, curve = Curves.parse_spec("hdd-zone=")
        assert key == ("hdd", -1, -1)
...
class TestCurvesGet:
    def test_exact_match(self) -> None:
        c = Curves({("gpu", 0, 1): ((50.0, 20.0), (80.0, 100.0))})
        assert c.get("gpu", 0, 1) == ((50.0, 20.0), (80.0, 100.0))
    ...
    def test_fallback_to_all_devices(self) -> None:
        c = Curves({("gpu", -1, 1): ((50.0, 20.0), (80.0, 100.0))})
        assert c.get("gpu", 0, 1) == ((50.0, 20.0), (80.0, 100.0))

    def test_default_gpu(self) -> None:
        c = Curves()
        assert c.get("gpu", 0, 0) is not None
        assert c.get("gpu", 0, 1) is not None

    def test_default_cpu_zone0_only(self) -> None:
        c = Curves()
        assert c.get("cpu", 0, 0) is not None
        assert c.get("cpu", 0, 1) is None


class TestCurvesLookup:
    def test_below_min(self) -> None:
        curve = ((40.0, 15.0), (80.0, 100.0))
        assert Curves.lookup(30, curve) == 15.0

    def test_above_max(self) -> None:
        curve = ((40.0, 15.0), (80.0, 100.0))
        assert Curves.lookup(90, curve) == 100.0

    def test_between_thresholds(self) -> None:
        curve = ((40.0, 15.0), (80.0, 100.0))
        assert Curves.lookup(60, curve) == 15.0  # piecewise constant: 60 >= 40, < 80
